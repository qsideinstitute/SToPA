...
def read_known_data_lists():
    ...
    """
    with open(f"{PROJECT_FOLDER}/data/williamstown_known_reasons.txt") as file:
        lines = file.readlines()
        reasons = [line.rstrip() for line in lines][1:]

    with open(f"{PROJECT_FOLDER}/data/williamstown_known_officers.txt") as file:
        lines = file.readlines()
        officers = [line.rstrip() for line in lines]

    with open(f"{PROJECT_FOLDER}/data/williamstown_known_actions.txt") as file:
        lines = file.readlines()
        actions = [line.rstrip() for line in lines]
    
    with open(f"{PROJECT_FOLDER}/data/williamstown_known_streets.txt") as file:
        lines = file.readlines()
